...
async function assertGuidedJourney(page, slug) {
  ...
  if (slug === "brokers-agencies") {
    ...
    assert.equal(await page.locator(".broker-official-pack, .broker-verification").count(), 0, "brokers must not repeat the selected task in a separate handoff section");
    assert.match(await page.locator(".broker-route-summary aside").innerText(), /A-01[\s\S]*Confirm eligibility and residency route[\s\S]*Open first task/i);
...
assert.match(await brokerJourney.locator(".broker-detail-actions-focused").innerText(), /Your next official action[\s\S]*goAML/i);
assert.equal(await brokerJourney.locator(".broker-official-pack, .broker-verification").count(), 0, "the official action must stay inside the selected task");
...
assert.equal(await brokerJourney.locator(".broker-directory-link").count(), 0, "Abu Dhabi must not inherit the Dubai brokerage directory");
...
await brokerJourney.locator(".broker-task-route > div > button").nth(1).click();
await brokerJourney.locator(".broker-detail-tabs button").nth(3).click();
assert.equal(await brokerJourney.locator(".broker-directory-link-focused").count(), 1, "Dubai agent task A-02 should offer the relevant brokerage verification link");
assert.match(await brokerJourney.locator(".broker-directory-link-focused").innerText(), /Verify the brokerage before joining/i);
