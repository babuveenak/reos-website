...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!parsed) return {};
  const group = groups.find((item) => item.id === parsed.stakeholderId)!;
  ...
  const canonical = `/stakeholders/${parsed.stakeholderId}${suffix}`;
  return { title: `${group.name} in ${emirate.label} | REOS`, description: `Jurisdiction-aware lifecycle structure for ${group.name} in ${emirate.label}, with explicit evidence and official-source boundaries.`, robots: parsed.emirate === "dubai" && parsed.stakeholderId === "landowners-investors" && parsed.track !== "financial-free-zone" ? undefined : { index: false, follow: true }, alternates: { canonical, languages: { en: canonical, ar: `/ar${canonical}` } } };
}

export default async function StakeholderJurisdictionPage({ params }: Props) {
  const parsed = parse(await params);
  if (!parsed) notFound();
  return <StakeholderBlueprintPage {...parsed} locale="en" />;
}
