...
type ExplorerMode = "journey" | "stakeholder" | "map";
...
export function JourneyStakeholderExplorer({ pathPrefix = "" }: { pathPrefix?: string }) {
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      ...
      if (modes.some((item) => item.id === requestedMode)) setMode(requestedMode!);
      else if (modes.some((item) => item.id === storedMode)) setMode(storedMode!);
  ...
  const chooseRelationship = (relationship: JourneyStakeholderRelationship) => {
    setSelectedStageId(relationship.stageId);
    setSelectedStakeholderId(relationship.stakeholderId);
  };
  const reset = () => {
    setSelectedStageId(null);
    setSelectedStakeholderId(null);
